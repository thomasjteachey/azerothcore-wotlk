...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    m_diminishGroup = GetDiminishingReturnsGroupForSpell(m_spellInfo, m_triggeredByAuraSpell.spellInfo || _triggeredCastFlags & TRIGGERED_FULL_MASK);
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
                    ...
                    if (m_spellInfo->HasAttribute(SPELL_ATTR0_HEARTBEAT_RESIST) && m_diminishGroup != DIMINISHING_NONE)
                    {
                        m_spellAura->SetHeartbeatDuration();
                    }
